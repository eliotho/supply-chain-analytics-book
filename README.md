# Supply Chain Analytics: A Data-Driven Approach
### By [Elioth Sanabria](https://eliotho.github.io/)

Welcome to the official repository for the draft chapters of **"Supply Chain Analytics: A Data-Driven Approach."** These notes are born out of a Supply Chain Analytics class that I taught at Columbia IEOR Department in Spring 2026. See the syllabus here: [Download the Syllabus](files/syllabus.pdf)

---

## 📢 Weekly Release Schedule
I am releasing chapters weekly to engage with the academic and practitioner communities!

- **Chapter 1:** [Demand Estimation and Forecasting](files/lecture_notes_ch_1.pdf)
- **Chapter 2:** [Inventory Management Models](files/lecture_notes_ch_2.pdf)
- **Chapter 3:** [Network Fulfillment Models](files/lecture_notes_ch_3.pdf)
- **Chapter 4:** [Scheduling Models](files/lecture_notes_ch_4.pdf) 
- **Chapter 5:** [Vehicle Routing Problems](files/lecture_notes_ch_5.pdf) (Available Now)
- **Chapter 6:** [Supply Chain Networks] (Coming Soon!)


- **Full book:** [Download Full Book](files/full_book.pdf)
---

## 🎓 How to Cite This Work
If you utilize the methodologies or concepts from this text in your research or classroom, please cite the work as follows:

### APA Style:
Sanabria, E. (2026). *Supply Chain Analytics: A Data-Driven Approach* (Draft Version). https://github.com/eliotho/supply-chain-analytics-book

### BibTeX for LaTeX Users:
```latex
@misc{Sanabria2026SCA,
  author = {Elioth Sanabria},
  title = {Supply Chain Analytics: A Data-Driven Approach},
  year = {2026},
  howpublished = {\url{[https://github.com/eliotho/supply-chain-analytics-book](https://github.com/eliotho/supply-chain-analytics-book)}},
}
```

---

## ⚖️ License & AI Training Notice
This work is licensed under the **Creative Commons Attribution-NonCommercial-NoDerivatives 4.0 International (CC BY-NC-ND 4.0)** license.

**⚠️ AI Training Prohibition:** Explicit permission is **NOT** granted for the contents of this repository (including PDFs, formulas, and code snippets) to be used for the training, fine-tuning, or testing of Large Language Models (LLMs) or generative AI systems. This is proprietary academic research.

---
